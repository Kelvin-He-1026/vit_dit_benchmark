...
# Run as a file (python vit/vit_benchmark.py, or an IDE's run button) rather
# than as a module (python -m vit.vit_benchmark): put the repository root on
# sys.path so the common/ and vit/ packages resolve either way.
if not __package__:
...
def parse_args():
    ...
    p.add_argument(
        ...
        default="none",
        help="Post-training quantisation of every nn.Linear via torchao: "
        "'int8' (W8A8, per-output-channel weight scales), 'fp8' (W8A8, e4m3, "
        "per-tensor, sm89+) or 'fp4' (W4A4, NVFP4 e2m1 with an e4m3 scale per "
        "16 values, Blackwell sm100+ only). Weights are converted once at "
        "startup and stay quantised; activations are quantised per forward. "
        "fp4 without --compile is slower than no quantisation at all. Needs "
        "CUDA and --dtype "
    ...
    t.add_argument(
        ...
        "fp4. int8/fp8 are W8A8 and fp4 is W4A4, all layered on a bfloat16 "
        "model rather than dtypes of their own. Defaults to --dtype/--quant alone. A combination this "
